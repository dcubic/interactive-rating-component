...
export default function SelectionWindow(props) {
  ...
  return (
    <main className={styles["background-panel"]}>
      ...
      <h1 className={`${styles["white-font"]} ${styles["header-size"]}`}>
        How did we do?
      </h1>
      <p className={`${styles["white-font"]} ${styles["paragraph"]}`}>
        ...
        {props.ratingValues.map((rating) => (
          <button
            ...
            className={`${styles["selection-button"]} ${
              props.ratingState === rating ? styles.selected : ""
            }`}
      ...
      <button onClick={handleSubmitClick} className={styles["submit-button"]}>
        Submit
      </button>
    </main>
  );
